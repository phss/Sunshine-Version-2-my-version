...
public class TestDb extends AndroidTestCase {
    ...
    */
    public void testLocationTable() {
        ...
        long rowId = db.insert(LocationEntry.TABLE_NAME, "", locationValues);
        assertFalse("Error: could not insert row", rowId == -1);

        // Query the database and receive a Cursor back
        Cursor cursor = db.query(LocationEntry.TABLE_NAME, null, LocationEntry._ID + "== ?", new String[]{rowId + ""}, null, null, null);

        // Move the cursor to a valid database row
        assertTrue("Error: no rows?", cursor.moveToFirst());

        // Validate data in resulting Cursor with the original ContentValues
        // (you can use the validateCurrentRecord function in TestUtilities to validate the
        // query if you like)
        assertEquals(TestUtilities.TEST_LOCATION, cursor.getString(1));
        assertEquals("North Pole", cursor.getString(2));
        assertEquals("-147.353", cursor.getString(3));
        assertEquals("64.7488", cursor.getString(4));
